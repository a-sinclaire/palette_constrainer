...
# SKITTLE 3
# This one is supposed to use all the colors up exactly.
...
def run(image, palette, Hue_Weight=1, Sat_Weight=1, Val_Weight=10, unlimited=False, random_order=True):
    ...
    # SETTING THE NUMBER OF "WHITE" SKITTLES TO EXACTLY
    # THE NUMBER OF EMPTY SPACES WE HAVE
    sum = 0
    for k in range(len(counts)):
        sum += counts[k]
    counts[5] = rows*cols-(sum - counts[5])
    print(counts)
...
run(image, skittles, Hue_Weight=H, Sat_Weight=S, Val_Weight=V, unlimited=False, random_order=True)
